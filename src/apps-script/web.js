...
function doPost(e) {
  ...
  if (action === 'getLabels') {
    ...
    return ContentService.createTextOutput(JSON.stringify({ ok: true, labels: result }))
      .setMimeType(ContentService.MimeType.JSON);
  }
  ...
  if (action === 'createLabel') {
    ...
    if (!labelName) {
      return ContentService.createTextOutput(JSON.stringify({ ok: false, error: '라벨 이름이 비어있습니다.' }))
        .setMimeType(ContentService.MimeType.JSON);
    }
    let label = GmailApp.getUserLabelByName(labelName);
    if (!label) label = GmailApp.createLabel(labelName);
    return ContentService.createTextOutput(JSON.stringify({ ok: true, name: label.getName() }))
      .setMimeType(ContentService.MimeType.JSON);
  }
  ...
  if (action === 'getLabelMessages') {
    ...
    if (!labelName) {
      return ContentService.createTextOutput(JSON.stringify({ ok: false, error: '라벨 이름이 필요합니다.' }))
        .setMimeType(ContentService.MimeType.JSON);
    }
    const label = GmailApp.getUserLabelByName(labelName);
    if (!label) {
      return ContentService.createTextOutput(JSON.stringify({ ok: false, error: '라벨을 찾을 수 없습니다.' }))
        .setMimeType(ContentService.MimeType.JSON);
    }
  ...
  if (action === 'getMessage') {
    ...
    if (!messageId) {
      return ContentService.createTextOutput(JSON.stringify({ ok: false, error: 'messageId가 필요합니다.' }))
        .setMimeType(ContentService.MimeType.JSON);
    }
    try {
  ...
  if (action === 'addLabelBySearch') {
    ...
    if (!labelName || messageIds.length === 0) {
      return ContentService.createTextOutput(JSON.stringify({ ok: false, error: '라벨명과 메일 ID가 필요합니다.' }))
        .setMimeType(ContentService.MimeType.JSON);
    }
