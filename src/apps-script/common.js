...
function _buildHomeCard() {
  var syncNewBtn = CardService.newTextButton()
    ...
    .setOnClickAction(CardService.newAction().setFunctionName("onSyncNewOnly"));

  var syncAllBtn = CardService.newTextButton()
    .setText("🔄 전체 갱신 (보낸 메일 포함)ㅤ")
    .setTextButtonStyle(CardService.TextButtonStyle.OUTLINED)
    .setOnClickAction(CardService.newAction().setFunctionName("onSyncAll"));
  ...
  var searchBtn = CardService.newTextButton()
    .setText("전송")
    .setTextButtonStyle(CardService.TextButtonStyle.OUTLINED)
    .setOnClickAction(CardService.newAction().setFunctionName("onSmartSearch"));
  ...
  return CardService.newCardBuilder()
    .setHeader(
      CardService.newCardHeader().setTitle("📮 메일을 스마트하게 관리하세요"),
    )
...
function _buildGmailMessageCard(e) {
  ...
  var calDesc = CardService.newTextParagraph().setText(
    "메일 본문에서 날짜/시간 정보를 추출하여 Google 캘린더에 일정을 등록합니다.",
  );
  ...
  return CardService.newCardBuilder()
    ...
    .setHeader(
      CardService.newCardHeader().setTitle("이 메일에 대한 작업을 선택하세요"),
    )
